...
class GPT(nn.Module):
    """Decoder-only GPT with RoPE + RMSNorm + SwiGLU + GQA — the modern
    baseline architecture every attention variant in 03-attention-variants/
    is compared against. Position comes entirely from RoPE (applied inside
    attention); there is no learned absolute position embedding table,
    unlike 01-baseline-smoke.
    """

...
class SwiGLU(nn.Module):
    """MoE-SwiGLU: top-k softmax routing over n_expert per-token experts
    with FIXED per-expert capacity (padded/truncated index tensors), so
    every forward uses identical tensor shapes -- the MPS allocator
    fragments catastrophically under variable shapes (measured in the
    DPO episode). Overflow tokens are dropped from expert compute (their
    residual path still carries them) and the drop rate is tracked.

    Train-time stats (EMA over optimizer steps, read by train.py at each
    eval): expert load fractions, drop rate, aux loss. The aux
    load-balance loss is exposed via self.last_aux and added to the CE
    loss by GPT.forward ONLY in training mode -- validation CE and the
    full-pass evaluator are never contaminated.

    Class name kept as SwiGLU so Block/GPT and the checkpoint tree stay
    structurally parallel to the dense arms."""
    ...
    def __init__(self, config: GPTConfig):
        ...
        self.experts = nn.ModuleList(
            [ExpertSwiGLU(config) for _ in range(config.n_expert)])
        self.dropout = nn.Dropout(config.dropout)
        self.last_aux = None
        # persistent: G-R health monitoring must survive --resume
        self.register_buffer("ema_load", torch.full((config.n_expert,),
                             1.0 / config.n_expert), persistent=True)
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            assign = F.one_hot(top_i, self.n_expert).sum(dim=1).float()  # (N, E)
            f = assign.mean(dim=0) / self.top_k  # slot fractions, sum = 1
        P = probs.mean(dim=0)
        # Switch eq. 4 (top-k generalized): E * sum_e f_e * P_e = 1 at uniform
        self.last_aux = (f.detach() * P).sum() * self.n_expert

        capacity = int(self.capacity_factor * N * self.top_k / self.n_expert)
        out = torch.zeros_like(flat)
        dropped = 0
        for e in range(self.n_expert):
            slot_mask = (top_i == e)                     # (N, k)
            gate = (top_p * slot_mask).sum(dim=-1)       # (N,) 0 where not routed
            routed = gate.nonzero(as_tuple=True)[0]
            n_routed = routed.numel()
            if n_routed > capacity:
                keep = gate[routed].topk(capacity).indices
                routed = routed[keep]
                dropped += n_routed - capacity
                n_routed = capacity
            # pad index tensor to FIXED capacity (pad entries get gate 0)
            idx = torch.zeros(capacity, dtype=torch.long, device=flat.device)
            idx[:n_routed] = routed
            g = torch.zeros(capacity, 1, device=flat.device, dtype=flat.dtype)
            g[:n_routed, 0] = gate[routed].to(flat.dtype)
            y = self.experts[e](flat.index_select(0, idx)) * g
            out = out.index_add(0, idx, y)
        if self.training:
            with torch.no_grad():
                self.ema_load.mul_(0.99).add_(0.01 * f)
                self.ema_drop.mul_(0.99).add_(0.01 * dropped / max(N * self.top_k, 1))
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is not None:
            ...
            if self.training:
                aux = sum(b.mlp.last_aux for b in self.blocks if b.mlp.last_aux is not None)

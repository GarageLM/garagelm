...
class GPTConfig:
    ...
    ffn_hidden: int = 1024  # PER-EXPERT SwiGLU hidden dim (top-2 x 1024 = dense-2048-equivalent active)
    n_expert: int = 8
    top_k: int = 2
    capacity_factor: float = 1.25  # fixed expert capacity -> fixed tensor shapes (MPS requirement)
    aux_loss_coeff: float = 0.01  # Switch-style load-balance loss (train-time only)

...
def main(resume=False):
    ...
    for it in range(start_iter, train_cfg.max_iters):
        if it % train_cfg.eval_interval == 0:
            ...
            loads = torch.stack([b.mlp.ema_load for b in model.blocks])
            drops = torch.stack([b.mlp.ema_drop for b in model.blocks])
            print(f"iter {it}: train_loss={losses['train']:.4f} val_loss={losses['val']:.4f} "
                  f"elapsed={elapsed_prior + time.time() - t0:.1f}s "
                  f"min_load={loads.min().item():.3f} drop={drops.mean().item():.3f}", flush=True)

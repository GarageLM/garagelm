...
def apply_rope(q, k, cos, sin):
    # q: (B, n_head, T, head_dim), k: (B, n_kv_head, T, head_dim)
...
class GQAAttention(nn.Module):
    """Named GQAAttention for continuity with 02's code -- with
    n_kv_head == n_head (set in config.py) this variant's repeat_kv becomes
    a no-op, making this plain full multi-head attention (MHA). This is the
    ONLY difference from 02-baseline-small-lm: everything else (RMSNorm,
    SwiGLU, RoPE, block structure) is byte-identical.
    """
...
class GPT(nn.Module):
    """Decoder-only GPT, identical to 02-baseline-small-lm except
    n_kv_head=n_head (full MHA instead of GQA) -- see config.py.
    """
    ...
    def __init__(self, config: GPTConfig):
        ...
        # GPT-2-style residual scaling: output projections get a smaller
        # init so residual stream variance doesn't blow up with depth.
    ...
    def num_params(self):
        # tok_emb and lm_head share the same Parameter object (weight tying);
        # nn.Module.parameters() already dedups identical Parameter objects,
        # so no manual subtraction is needed here.

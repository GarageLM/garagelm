...
MUON_LR = 0.02  # speedrun-standard for hidden matrices; cosine-scaled with the AdamW lr


class DualOptimizer:
    """AdamW (embeddings + norms) and Muon (hidden 2D matrices) behind the
    single-optimizer interface train.py already uses; both are scheduled by
    one multiplicative factor so the cosine shape matches the control."""

    def __init__(self, adamw, muon):
        self.opts = [adamw, muon]
    ...
    def state_dict(self):
        return {"adamw": self.opts[0].state_dict(), "muon": self.opts[1].state_dict()}

    def load_state_dict(self, sd):
        self.opts[0].load_state_dict(sd["adamw"])
        self.opts[1].load_state_dict(sd["muon"])

ROOT = os.path.dirname(__file__)
DATA_DIR = os.path.join(ROOT, "..", "..", "05-data-frontier", "data")  # shared 05 pool
...
def build():
    ...
    # hidden 2D matrices -> Muon; embeddings (tied lm_head) + 1D norm gains -> AdamW
    muon_params = [p for n, p in model.named_parameters() if p.ndim == 2 and "tok_emb" not in n]
    adamw_params = [p for n, p in model.named_parameters() if not (p.ndim == 2 and "tok_emb" not in n)]
    assert len(muon_params) == 7 * model_cfg.n_layer, len(muon_params)
    optimizer = DualOptimizer(
        torch.optim.AdamW(adamw_params, lr=train_cfg.learning_rate,
                          weight_decay=train_cfg.weight_decay),
        Muon(muon_params, lr=MUON_LR),
    )
...
def main(resume=False):
    ...
    for it in range(start_iter, train_cfg.max_iters):
        ...
        optimizer.set_lr_factor(get_lr(it, train_cfg) / train_cfg.learning_rate)

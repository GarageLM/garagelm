...
DATA_DIR = os.path.join(ROOT, "data")  # this lane's 25M-token sub-pool (see data.py)
...
def load_data():
    # Multi-epoch arm: train on the 25M-token stratified sub-pool built by
    # data.py (100M-token budget / 25M pool = ~4 epochs), val unchanged
    # from the 05 pool so losses compare directly with the control.
    pool_05 = os.path.join(ROOT, "..", "..", "05-data-frontier", "data")
    with open(os.path.join(pool_05, "meta.pkl"), "rb") as f:
        meta = pickle.load(f)
    train_ids = np.memmap(os.path.join(DATA_DIR, "subpool.bin"), dtype=np.uint16, mode="r")
    val_ids = np.memmap(os.path.join(pool_05, "val.bin"), dtype=np.uint16, mode="r")

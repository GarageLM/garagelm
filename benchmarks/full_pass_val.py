...
@torch.no_grad()
def full_pass_loss(model, tokens, device, block, micro=8, mask=None):
    """Unmasked CE by default; with `mask` (uint8, parallel to tokens),
    masked CE = sum(CE*mask)/sum(mask) over the same deterministic
    windows -- the pinned evaluator for assistant-masked SFT gates."""
    ...
    for w0 in range(0, n_win, micro):
        ...
        for w in range(w0, min(w0 + micro, n_win)):
            ...
            if mask is not None:
                ms.append(torch.from_numpy(mask[k + 1:k + 1 + block].astype(np.float32)))
        ...
        if mask is not None:
            m = torch.stack(ms).to(device).view(-1)
            total += (ce * m).sum().item()
            count += m.sum().item()
        else:
            total += ce.sum().item()
...
def main():
    ...
    p.add_argument("--mask-bin", default=None,
                   help="optional uint8 loss mask parallel to val-bin (masked CE)")
    ...
    mask = (np.memmap(args.mask_bin, dtype=np.uint8, mode="r")
            if args.mask_bin else None)
    if mask is not None:
        assert len(mask) == len(tokens), "mask/tokens length mismatch"
    loss = full_pass_loss(model, tokens, device, cfg.block_size, args.micro, mask=mask)

    name = result_name(exp_dir)
    out = {"experiment": name, "full_pass_val_loss": round(loss, 4),
           "masked": args.mask_bin is not None,
